...
public class Common {
    ...
    public static PokerType jugdeType(ArrayList<Poker> list) {
        ...
        if (len <= 4) {
            ...
            //因为出的牌已经排好序的，所以如果出牌的第一张牌和最后一张牌相同，说明这几张牌都是相同的，可能是单牌、对子、3不带、炸弹。
            if (!list.isEmpty() && Common.getValue(list.get(0)) == Common.getValue(list.get(len - 1))) {
                switch (len) {
                    case 1://len = 1，1张牌，单牌
                        return PokerType.c1;
                    case 2://len = 2，2张牌，对子
                        return PokerType.c2;
                    case 3://len = 3，3张牌，三不带
                        return PokerType.c3;
                    case 4://len = 4，4张牌，炸弹
                        return PokerType.c4;
                }
